...
def parse_text_file(path: str | Path) -> tuple[list[ReportSection], list[ReportUnit], list[Table], list[Citation]]:
    source = Path(path)
    text = source.read_text(encoding="utf-8")
    sections, units = segment_text(text, source, markdown=False)
